	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 8rpx 0px 4rpx var(--publicMainColor)","margin":"0","borderColor":"rgba(0,0,0,.6)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"20rpx 20rpx 0 0","borderWidth":"0","fontSize":"28rpx","borderStyle":"solid","height":"60rpx"}'
						v-model="searchForm.xianhuaName" type="text" placeholder="鲜花名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('xianhua','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('xianhua','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='xianhuaTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in xianhuaTypesList" :key="index" :class="xianhuaTypes===item.codeIndex?'tabActive':''"
                                  @tap="xianhuaTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.xianhuaPhoto?baseUrl+product.xianhuaPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.xianhuaName}}
                </view>
                <view v-if="product.xianhuaNewMoney"
                      ...
                      class="title hide1">￥{{product.xianhuaNewMoney}}
                </view>
                <view v-else-if="product.xianhuaValue"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title hide1">{{product.xianhuaValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx">
                    <text v-if="isAuth('xianhua','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('xianhua','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 鲜花类型 字段 字典表查询方法
            async xianhuaTypesSelect(){
                let xianhuaTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'xianhua_types',
                });
                xianhuaTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.xianhuaTypesList = xianhuaTypes.data.list
            },
            //当前表的 鲜花类型字段类别搜索
			xianhuaTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xianhuaTypes != 0) {
                    params.xianhuaTypes = this.xianhuaTypes
                }
                let _this = this
                let res = await this.$api.list(`xianhua`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xianhua', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`xianhua`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.xianhuaName) {
                    searchForm['xianhuaName'] = this.searchForm.xianhuaName
                }

                let res = await this.$api.list(`xianhua`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.xianhuaContent!=null){
                        item.xianhuaContent = item.xianhuaContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
