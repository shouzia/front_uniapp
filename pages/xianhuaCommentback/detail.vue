	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xianhuaCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('xianhuaCommentback', this.xianhuaCommentbackId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xianhuaCommentbackPhoto ? this.detail.xianhuaCommentbackPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xianhuaCommentbackYuyue', {
