        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.xianhuaCommentbackText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`xianhuaCommentback`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.xianhuaId){
                this.ruleForm.xianhuaId = options.xianhuaId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.xianhuaCommentbackText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`xianhuaCommentback`, this.ruleForm);
                } else {
                    await this.$api.save(`xianhuaCommentback`, this.ruleForm);
                }
