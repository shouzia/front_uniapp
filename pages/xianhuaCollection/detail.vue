	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xianhuaCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('xianhuaCollection', this.xianhuaCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xianhuaCollectionPhoto ? this.detail.xianhuaCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xianhuaCollectionYuyue', {
