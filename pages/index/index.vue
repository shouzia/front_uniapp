  ...
  <view class="uni-padding-wrap">
    <view class="header">
      <view class="headerb">
        <swiper
          ...
          class="swiper"
          :indicator-dots="'.swiper-pagination' == null ? false : true"
    ...
    >
      <block v-for="item in menuList" v-bind:key="item.roleName">
        <block
          v-if="
            (role == item.roleName || table == item.tableName) &&
            index <= 4 &&
            index > 0
          "
          v-bind:key="index"
            ...
            <block v-for="(button, sort2) in child.buttons">
              <view
                :style="{
                  padding: '10rpx',
                  margin: '0 2% 20rpx 2%',
                  borderColor: 'rgba(244, 245, 244, 1)',
    ...
    <view class="listBox recommend" style="height: auto">
      ...
      >
        <div
          @tap="onDetailTap('xianhua', item.id)"
          v-for="(item, index) in xianhuaList"
          :key="index"
        ...
        >
          ...
          <div style="padding: 0px 5px">
            ...
            <div style="color: red; margin-top: 10px">
              ￥<span style="font-size: 16px">{{ item.xianhuaOldMoney }}</span>
            </div>
          </div>
        </div>
      </view>
...
export default {
  ...
  async onShow() {
    ...
    let xianhua = await this.$api.requestCondition(
      'xianhua',
      'gexingtuijian',
      xianhuaVal
    )
    ...
    this.xianhuaList.forEach(function (item, index) {
      if (
        item.xianhuaContent != null &&
        item.xianhuaContent != '' &&
        item.xianhuaContent != 'null'
      ) {
        item.xianhuaContent = item.xianhuaContent.replace(
          /<img [^>]*src=['"]([^'"]+)[^>]*>/gi,
          ''
        ) //替换图片
      }
    ...
    this.gonggaoList.forEach(function (item, index) {
      if (
        item.gonggaoContent != null &&
        item.gonggaoContent != '' &&
        item.gonggaoContent != 'null'
      ) {
        item.gonggaoContent = item.gonggaoContent.replace(
          /<img [^>]*src=['"]([^'"]+)[^>]*>/gi,
          ''
        ) //替换图片
      }
