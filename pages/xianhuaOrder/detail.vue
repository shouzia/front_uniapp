	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xianhuaOrderId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('xianhuaOrder', this.xianhuaOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xianhuaOrderPhoto ? this.detail.xianhuaOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xianhuaOrderYuyue', {
									yonghuId: _this.user.id,
									xianhuaOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xianhuaOrderYuyue/list');
							}
						}
					});
				},
